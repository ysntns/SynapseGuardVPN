...
class VpnRepositoryImpl @Inject constructor(
    ...
    override suspend fun connect(config: ConnectionConfig): Result<Unit> = withContext(ioDispatcher) {
        try {
            ...
            val terminalState = withTimeoutOrNull(CONNECTION_TIMEOUT_MS) {
                _vpnState
                    .filter { state ->
                        state !is VpnState.Connecting && state !is VpnState.Idle
                    }
                    .first()
            }
            ...
            when (terminalState) {
                ...
                is VpnState.Error -> {
                    Result.failure(Exception(terminalState.message))
                }

                null -> {
                    Timber.w("VPN connection timed out while waiting for terminal state")
                    context.startService(
                        Intent(context, VpnConnectionService::class.java).apply {
                            action = VpnConnectionService.ACTION_DISCONNECT
                        }
                    )
                    _vpnState.value = VpnState.Error("VPN connection timed out. Lütfen tekrar deneyin.")
                    Result.failure(TimeoutCancellationException("VPN connection timed out"))
                }

                else -> {
                    // Connection is still in progress; let the UI continue to observe state
                    Result.success(Unit)
                }
            }
